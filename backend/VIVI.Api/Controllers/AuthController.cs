...
public sealed class AuthController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var email = $"customer.{phone}@vivicrochet.dev".ToLowerInvariant();
        var user = await _db.AdminUsers.SingleOrDefaultAsync(u => u.Email == email, cancellationToken);
        ...
        if (user is null)
        {
            user = new AdminUser
            {
                Id = Guid.NewGuid(),
                Email = email,
                Name = string.IsNullOrWhiteSpace(request.Name) ? "VIVI Learner" : request.Name.Trim(),
        ...
        CancellationToken cancellationToken)
    {
        ...
        var user = await _customers.GetOrCreateAsync(challenge.Phone, request.Name, cancellationToken);
        var token = _jwt.CreateAccessToken(user);
        return Ok(new LoginResponse
        {
            AccessToken = token.AccessToken,
