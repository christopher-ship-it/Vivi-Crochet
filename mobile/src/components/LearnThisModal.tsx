...
export function LearnThisModal({
  ...
  const lessonLabel =
    course.videoCount === 1
      ? t('learnThisModal.lessonOne')
      : t('learnThisModal.lessonsCount', { count: course.videoCount });
  const meta = [course.level, lessonLabel, t('learnThisModal.priceFrom', { price: formatInr(course.price) })]
    .filter(Boolean)
    .join(' · ');
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onDismiss}>
      <Pressable style={styles.backdrop} onPress={onDismiss}>
        <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
          ...
          <Text style={styles.eyebrow}>{t('learnThisModal.eyebrow')}</Text>
          <Text style={styles.title}>{t('learnThisModal.title')}</Text>
          <Text style={styles.body}>{t('learnThisModal.body', { productName })}</Text>
          ...
          <Pressable style={styles.primaryBtn} onPress={onLearn}>
            <Text style={styles.primaryText}>{t('learnThisModal.cta')}</Text>
          </Pressable>
          <Pressable style={styles.secondaryBtn} onPress={onDismiss}>
            <Text style={styles.secondaryText}>{t('learnThisModal.notNow')}</Text>
          </Pressable>
        </Pressable>
      </Pressable>
    </Modal>
